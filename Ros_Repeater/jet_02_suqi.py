...
####################舵机基础功能####################
    ...
    def set_zero_point(self, motor_id):
        data_array = (c_ubyte * 8)(0xC4, motor_id, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00)
        vci_can_obj = self.VCI_CAN_OBJ(motor_id, 0, 0, 1, 0, 0, 8, data_array)
        self.send_command(vci_can_obj)
        self.receive_data()
...
controller = CANMotorController(motor_id=0x02)
# 下发电机运动
...
# 初始化电机，设置零点和新ID 正反转
# controller.initialize_motor(motor_id=0x02, new_id=0x02)
# 读取当前电机信息
# controller.state_motor()
