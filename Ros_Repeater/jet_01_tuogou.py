...
class CANMotorController:
    ...
    def open_device(self):
        self.canDLL = cdll.LoadLibrary('./86_libcontrolcan.so')
...
# 初始化电机，设置零点和新ID 正反转
    def initialize_motor(self, motor_id, new_id):
        ...
        for _ in range(2):
            ...
            time.sleep(1)
            print("success")
            self.set_max_angle(motor_id, angle=270, direction='positive')
            self.set_max_angle(motor_id, angle=-90, direction='negative')
...
controller = CANMotorController(motor_id=0x03)
# 下发电机运动
...
# 初始化电机，设置零点和新ID 正反转
# controller.initialize_motor(motor_id=0x03, new_id=0x03)
