...
class CANMotorController:
    ...
    def init_can_channel(self):
        vci_initconfig = self.VCI_INIT_CONFIG(0x80000008, 0xFFFFFFFF, 0, 0, 0x00, 0x14, 0)  # 波特率500k，正常模式
    ...
    def send_command(self, vci_can_obj):
        ...
        if ret == self.STATUS_OK:
            # 输出16进制格式的Data内容
            hex_data = [f'{byte:02X}' for byte in vci_can_obj.Data]
            print(f'命令发送成功: {hex_data}')
        else:
...
####################舵机基础功能####################
    ...
    def send_angle(self, motor_id, angle):
        motor_id=0x04
        motor_mode = 0x01  # 电机模式，表示伺服位置控制模式
        # 将角度从度数转换为 IEEE 754 float32 格式
        pos = struct.unpack('>I', struct.pack('>f', float(angle)))[0]  # 转换为float32的二进制表示

        # 期望速度设置为10%的最大速度，最大值为3276.7rpm
        spd = int(150)  # 10%的速度
        
        # 电流阈值设置为50%的最大电流，最大值为409.5A
        cur = int(70)  # 50%的电流
        ...
        data_array = (c_ubyte * 8)(
            (motor_mode << 5) | ((pos >> 27) & 0x1F),   # 第1字节：电机模式 (3位) + 期望位置的高5位
            (pos >> 19) & 0xFF,                         # 第2字节：期望位置的接下来的8位
            (pos >> 11) & 0xFF,                         # 第3字节：期望位置的中间8位
            (pos >> 3) & 0xFF,                          # 第4字节：期望位置的低8位
            ((pos & 0x7) << 5) | ((spd >> 10) & 0x1F),  # 第5字节：期望位置的最后3位 + 期望速度的高5位
            (spd >> 2) & 0xFF,                          # 第6字节：期望速度的中间8位
            ((spd & 0x3) << 6) | ((cur >> 6) & 0x3F),   # 第7字节：期望速度的低2位 + 电流阈值的高6位
            ((cur & 0x3F) << 2) | (ack & 0x3)           # 第8字节：电流阈值的低6位 + 报文返回状态 (2位)
        )
