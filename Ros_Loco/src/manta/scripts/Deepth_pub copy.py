...
class PIDDepthController:
    ...
    def control_step(self):
        ...
        # 将控制信号限制在300到500之间
        control_signal = max(min(int(control_signal), 500), 300)  # 确保control_signal为整数类型
...
if __name__ == '__main__':
    ...
    # 本地定义目标深度
    target_depth = 1.0  # 目标深度为1米

    # 创建PID控制器实例，执行时间为10秒
    controller = PIDDepthController(target_depth=target_depth, kp=1.0, ki=0.01, kd=0.1, duration=10.0)
