...
class PIDDepthController:
    def __init__(self, target_depth, kp=1.0, ki=0.01, kd=0.1, duration=10.0):
        ...
        # ROS订阅和发布
        self.depth_sub = rospy.Subscriber('altitude_sensor_data', Warmdepth, self.depth_callback)
    ...
    def depth_callback(self, msg): 
        self.current_depth = msg.height
        self.depth_count += 1
        #rospy.loginfo(f"Current Hight: {self.current_depth}")
    ...
    def control_step(self):
        ...
        # 将控制信号限制在300到500之间
        control_signal = max(min(int(control_signal), 2200), -2200)  # 确保control_signal为整数类型
        ...
        if current_time - self.last_print_time >= 1.0:
            rospy.loginfo(f"Current Hight: {self.current_depth}, Target Hight: {self.target_depth}, Control Signal: {control_signal}")
...
if __name__ == '__main__':
    ...
    # 本地定义目标深度
    target_depth = 2# 目标深度为1米

    # 创建PID控制器实例，执行时间为10秒
    controller = PIDDepthController(target_depth=target_depth, kp=8000.0, ki=500, kd=0, duration=180.0)
