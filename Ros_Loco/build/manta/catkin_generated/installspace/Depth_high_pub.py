...
import rospy
from sensor_fish.msg import Warmdepth as HeightMsg  # 高度传感器消息类型
...
class HeightController:
    ...
    def trigger_depth_control(self):
        # 初始化并启动深度控制器
        depth_controller = DepthController(target_depth=2.0, kp=1.0, ki=0.01, kd=0.1, duration=10.0)
...
class DepthController:
    def __init__(self, target_depth=2.0, kp=1.0, ki=0.01, kd=0.1, duration=10.0):
        ...
        # 订阅深度传感器数据
        self.depth_sub = rospy.Subscriber('depth_sensor_data', DepthMsg, self.depth_callback)
        # 发布控制信号
    ...
    def control_step(self):
        ...
        # 将控制信号限制在300到500之间
        control_signal = max(min(int(control_signal), 500), 300)
...
def main():
    ...
    # 启动高度控制器
    height_controller = HeightController(target_height=10.0)
