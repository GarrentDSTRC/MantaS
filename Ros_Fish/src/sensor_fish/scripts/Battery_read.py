...
from sensor_fish.msg import BatteryStatus  # 导入自定义消息类型
import pandas as pd
...
def main():
    ...
    battery_status_pub = rospy.Publisher('/battery/status', BatteryStatus, queue_size=10)
    ...
    df = pd.DataFrame(columns=["Time", "TotalVoltage", "SOC", "SOH", "RelayStatus"])
    ...
    try:
        while not rospy.is_shutdown():
            ...
            if relay_status is not None:
                ...
                relay_status_str = ', '.join([f"{key}: {'开' if val else '关'}" for key, val in relay_status_parsed.items()])
                battery_status_msg.relay_status = relay_status_str
            ...
            if total_voltage is not None:
                ...
                battery_status_msg.total_voltage = total_voltage_parsed
            ...
            if soc is not None:
                ...
                battery_status_msg.soc = soc_percentage
            ...
            if soh is not None:
                ...
                battery_status_msg.soh = soh_percentage
                print(f"SOH: {soh_percentage:.1f}%")
            else:
                print("No SOH received or CRC error.")

            # 发布消息
            battery_status_pub.publish(battery_status_msg)

            # 保存数据到DataFrame
            current_time = rospy.get_time()
            df.loc[len(df)] = [current_time, total_voltage_parsed, soc_percentage, soh_percentage, relay_status_str]
        ...
        ser.close()
        df.to_csv("battery_data_log.csv", index=False)
        print("Data saved to battery_data_log.csv")
