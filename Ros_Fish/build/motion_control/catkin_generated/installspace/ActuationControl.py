...
# 0 表示左旋， 1表示右旋
vis = [0, 0, 1, 1, 1, 1, 1, 1]
ID=[0x0360,0x0361,0x0364,0x0365,0x0313,0x0314]
...
# a = ubyte_array(0x54, 0x43, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00)
...
def getVelocityM(v):
    ...
    #return ubyte_array(0x54, 0x43, 0x00, 0x00, 0x00, 0x00, 0x00, v)
    if v>=0:
...
def handle_command(data, device_type):
    ...
    if device_type == 'propeller':
    	vci_can_obj = VCI_CAN_OBJ(ID[pid], 0, 0, 1, 0, 0, 8, a, b)  # 单次发送
    if device_type == 'motor':
    	vci_can_obj = VCI_CAN_OBJ(0x0300 | pid, 0, 0, 1, 0, 0, 8, a, b)  # 单次发送
...
def receiveData():
    ...
    #rospy.init_node('actuation_control_node', anonymous=True) 
